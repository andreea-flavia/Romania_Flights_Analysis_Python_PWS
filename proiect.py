...
#     'missing_values': df_ro.isna().sum(),
#     'missing_pct': (df_ro.isna().mean() * 100).round(2),
#     'n_unique': df_ro.nunique()
# })
# # print(quality)
#
...
# df_ro['is_multi_airline'] = df_ro['airline'].str.contains(r' \+ ', regex=True)
...
#
#
# # 1. Do prices rise closer to departure?
# # This is probably the first question most travelers would ask.
#
# lead_curve = df.groupby('days_left', as_index=False)['price'].agg(['median', 'mean', 'count']).reset_index()
#
# plt.figure(figsize=(14, 6))
# sns.lineplot(data=lead_curve, x='days_left', y='median', linewidth=3, label='Median price')
# sns.lineplot(data=lead_curve, x='days_left', y='mean', linewidth=2, alpha=0.7, label='Mean price')
# plt.title('Flight prices rise as departure approaches')
# plt.xlabel('Days left until departure')
# plt.ylabel('Price (USD)')
# plt.show()
#
# # 2. Are connecting flights cheaper than nonstops?
# # People often assume connections save money. Let's test that directly.
#
# stop_summary = df.groupby('stops', as_index=False)['price'].agg(['median', 'mean', 'count']).reset_index()
# stop_summary
# plt.figure(figsize=(12, 6))
# sns.boxplot(data=df[df['stops'] <= 3], x='stops', y='price', showfliers=False)
# plt.title('Connecting itineraries are often more expensive, not less')
# plt.xlabel('Stops')
# plt.ylabel('Price (USD)')
# plt.show()
#
# # 3. How much does duration matter?
# plt.figure(figsize=(12, 6))
# sns.scatterplot(
#     data=df.sample(min(25000, len(df)), random_state=42),
#     x='duration_minutes', y='price', hue='stops', alpha=0.25, palette='viridis'
# )
# plt.title('Longer itineraries tend to be more expensive')
...
#     df['duration_minutes'],
#     bins=[0, 90, 180, 300, 2000],
#     labels=['<1.5h', '1.5-3h', '3-5h', '>5h']
# )
# duration_summary = df.groupby(duration_bins)['price'].median().reset_index()
# duration_summary
#
# # 4. 4. Which country-pairs look expensive?
# country_pair_summary = (
#     df.groupby('country_pair')
...
# sns.barplot(data=country_pair_summary, y='country_pair', x='median_price', palette='rocket')
# plt.title('Most expensive country-pairs by median observed fare')
# plt.xlabel('Median price (USD)')
# plt.ylabel('Country pair')
# plt.show()
#
#
# # 5. Which airlines look cheap or expensive?
# airline_summary = (
#     df.groupby('airline')
#       .agg(rows=('price', 'size'), median_price=('price', 'median'), mean_price=('price', 'mean'))
#       .query('rows >= 300')
...
# )
#
# cheap_airlines = airline_summary.head(12).reset_index()
# expensive_airlines = airline_summary.sort_values('median_price', ascending=False).head(12).reset_index()
#
# display(cheap_airlines)
# display(expensive_airlines)
#
# # 6. Does time of day matter?
# daypart_summary = (
#     df.groupby('departure_daypart')
#       .agg(rows=('price', 'size'), median_price=('price', 'median'), mean_price=('price', 'mean'))
...
# sns.barplot(data=daypart_summary, x='departure_daypart', y='median_price', palette='crest')
# plt.title('Median fare by departure part of day')
# plt.xlabel('Departure daypart')
# plt.ylabel('Median price (USD)')
# plt.show()
#
#
# # 8. A simple price model
# # This is not meant to be a production model. It is a quick test of whether the dataset contains enough signal to predict fare with reasonable structure.
#
# model_df = df.copy()
# features = [
#     'days_left', 'stops', 'duration_minutes',
#     'source_country', 'destination_country',
#     'departure_daypart', 'arrival_daypart',
...
#
# categorical = ['source_country', 'destination_country', 'departure_daypart', 'arrival_daypart', 'source_airport', 'destination_airport']
# numeric = ['days_left', 'stops', 'duration_minutes']
#
...
#
# # This makes the dataset useful not just for visualization, but for meaningful price analysis and predictive modeling